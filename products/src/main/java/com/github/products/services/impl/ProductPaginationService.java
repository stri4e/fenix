...
public class ProductPaginationService implements IProductPaginationService {
    ...
    @Override
    public Page<Product> searchProduct(String text) {
        return Optional.ofNullable(this.productSearchRepo.keyWorldsSearch(text))
                .orElse(this.productSearchRepo.fuzzySearch(text));
    }
